...
void PlayingMedia::playCurrentTrack(){
    ...
    if(!currentplaylist.empty()){
        ...
        if (currentplaylist[currentTrackIndex]->getType() == MediaType::AUDIO) {
            ...
            playVideo(currentTrackPath.c_str());
        }
    } else {
...
void audioCallback(void* userdata, uint8_t* stream, int len) {
    auto* playingMedia = static_cast<PlayingMedia*>(userdata); // Lấy đối tượng PlayingMedia
    AVCodecContext* audioCodecContext = playingMedia->audioCodecContext;

    int volume = playingMedia->getVolume(); // Lấy giá trị volume
    float volumeFactor = (float)volume / 128.0f; // Tỷ lệ âm lượng
    ...
    while (len_in_samples > 0) {
        ...
        for (int i = 0; i < amount / sizeof(int16_t); ++i) {
            int16_t sample = reinterpret_cast<int16_t*>(audioBuffer + audioBufferIndex)[i];
            sample = static_cast<int16_t>(sample * (playingMedia->getVolume() / 128.0f)); // Áp dụng tỷ lệ volume
            out_stream[i] = sample;
        }
...
void PlayingMedia::playVideo(const char* filePath) {
    ...
    // Audio codec
    this->audioCodecContext = nullptr; // Đảm bảo reset nếu trước đó có dữ liệu
    if (audioStreamIndex != -1) {
        ...
        if (audioCodec) {
            ...
            avcodec_parameters_to_context(this->audioCodecContext, audioCodecParams);
            if (avcodec_open2(this->audioCodecContext, audioCodec, nullptr) < 0) {
    ...
    wanted_spec.callback = this->audioCodecContext ? audioCallback : nullptr;
    ...
    if (this->audioCodecContext) {
        ...
        std::cerr << "Audio context is null. Audio playback is disabled.\n";
    }
    ...
    while (av_read_frame(formatContext, &packet) >= 0 && !quit) {
        ...
        else if (packet.stream_index == audioStreamIndex && this->audioCodecContext) {
